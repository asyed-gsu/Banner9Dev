	...
	public void keyBlock_blockOut()
	{
	  ...
	  @ActionTrigger(action="CHECK_KEYS")
		public void keyBlock_CheckKeys()
		{
				...
				getTask().getGoqrpls().gCheckValue(!(!getFormModel().getKeyBlock().getPidm().isNull() && !getFormModel().getKeyBlock().getId().isNull()), GNls.Fget(toStr("WHAWITD-0004"), toStr("FORM"), toStr("*ERROR* A valid ID must be entered before proceeding.")), toBool(NBool.True), toBool(NBool.True));
		}
		...
		public void id_validate()
		{
			...
			try {
				...
				getTask().getGoqrpls().gCheckValue(toBool(ptiCursor.notFound()),
						GNls.Fget(toStr("Whawitd-0001"), toStr("FORM"),
